...
/* $Id: playaudio.cpp 178 2011-04-29 17:45:41Z too-tired $ */
...
public:
    ...
    void play(int channels, int samplerate, int16_t *data, int bytes)
    {
        if ((not m_device) or channels!=m_channels or samplerate!=m_samplerate)
        {
            ...
            if (!m_device)
                throw tvclipper_exception("Error setting up audio output");
        }
...
public:
    a52dec() : m_state(0)
    {
        ...
        if (not m_state)
            throw tvclipper_exception("Error setting up AC3 decoder");
    }
    ~a52dec()
    {
        a52_free(m_state);
...
void playaudio_ac3(const void *data, uint32_t len)
{
    ...
    while (len>=7)
    {
        ...
        if (dec.frame(d,flags,level,0)!=0)
            throw tvclipper_exception("Error while decoding AC3 data");
        if ((flags&A52_CHANNEL_MASK)!=A52_STEREO)
            throw tvclipper_exception("Error while decoding AC3 data (non-stereo output)");
        for (int i=0;i<6;++i)
        {
            if (dec.block()!=0)
                tvclipper_exception("Error while decoding AC3 data");
    ...
    //   throw tvclipper_exception("AC3 playback not yet implemented");
}
#else // HAVE_LIB_A52
void playaudio_ac3(const void *, uint32_t)
{
    throw tvclipper_exception("TVCLIPPER was built without AC3 support");
}
...
void playaudio_mp2(const void *data, uint32_t len)
{
    throw tvclipper_exception("TVCLIPPER was built without MP2 support");
}
#endif // ifdef HAVE_LIB_MAD

// If TVCLIPPER is compiled without audio support, these dummy functions
...
void playaudio_ac3(const void *, uint32_t)
{
    throw tvclipper_exception("TVCLIPPER was built without support for audio output");
}
