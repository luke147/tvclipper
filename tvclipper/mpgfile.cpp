...
mpgfile* mpgfile::open(inbuffer *b, QString *errormessage) {
    ...
    for (int stride = TSPACKETSIZE; stride <= MAXPACKETSIZE; stride += 4) {
        initialoffset = tsfile::probe(*b, stride);
        if (initialoffset >= 0) // is this an mpeg transport stream?
            return new tsfile(*b, initialoffset, stride);
    }
    initialoffset = psfile::probe(*b);
    if (initialoffset >= 0) // is this an mpeg program stream?
...
void mpgfile::playaudio(int aud, int picture, int ms)
{
    ...
    while (stopreadpic<pictures-1 && idx[stopreadpic].getpts()<stoppts+180000)
        ++stopreadpic;
    tvclipper_off_t stopreadpos=idx[stopreadpic].getpos().packetposition();
...
void mpgfile::savempg(muxer &mux, int start, int stop, int savedpics, int savepics, logoutput *log)
{
    ...
    {
        tvclipper_off_t start_pos = idx[idx.indexnr(start)].getpos().fileposition();
        tvclipper_off_t stop_pos = idx[idx.indexnr(stop)].getpos().fileposition();
        tvclipper_off_t bytes = stop_pos - start_pos;
    ...
    for (int a=0;a<MAXAUDIOSTREAMS;++a) {
        streamdata *sd=sh.stream[audiostream(a)];
        if (!sd)
            continue;
    ...
    while (!log || !log->cancelled())
    {
        ...
        if (vsd)
            for(;;)
            {
                ...
                if (log && savepics > 0) {
                    int progress = ++savedpics * 1000 / savepics;
...
void mpgfile::recodevideo(muxer &mux, int start, int stop, pts_t offset,int savedpics,int savepics, logoutput *log)
{
    ...
    while (outpicture<stop)
    {
        ...
        int got_packet_ptr;
        av_init_packet(&avpkt);
        avpkt.data = (u_int8_t*)m2v.writeptr();
        ...
        if (!framelist.empty())
        {
            ...
            encodeError = avcodec_encode_video2(avcc, &avpkt, f, &got_packet_ptr);
        ...
        mux.setdts(VIDEOSTREAM,vidpts);
        fixtimecode(avpkt.data,encodeError,vidpts);
        mux.putpacket(VIDEOSTREAM,avpkt.data,encodeError,vidpts,viddts,
...
ssize_t mpgfile::readfile(QString filename, uint8_t **buffer) {
    ...
    *buffer = (uint8_t*) calloc(lSize, sizeof(uint8_t));
    if (buffer == NULL) {
        fclose(pFile);
        return -2;
    }
