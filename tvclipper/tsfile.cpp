...
/* $Id: tsfile.cpp 176 2011-04-25 00:44:30Z too-tired $ */
...
int tsfile::isTOPFIELD(const uint8_t *header, int len, std::string recfile) {  
...
tsfile::isTF7700HDPVR(std::string recfile) {  
  ...
  if(len>=boff+4) { 
      ...
      while ((bookmark=(buffer[boff+3]<<24)|(buffer[boff+2]<<16)|(buffer[boff+1]<<8)|buffer[boff])
          ...
          if(verbose)
              fprintf(stderr,"BOOKMARK[%d] = %" PRINT_FORMAT_PTS_T "\n", bnum, bookmark);
      ...
      fprintf(stderr,"ADD-File probabely corrupted (%zd bytes to short), discarding bookmarks!\n", TF7700HDPVR_LEN - len);
  }
